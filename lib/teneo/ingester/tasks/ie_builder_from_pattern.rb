    ...
    module Tasks
      ...
      class IeBuilderFromPattern < Teneo::Ingester::Tasks::Base::Task
        ...
        help_text <<~STR
          ...
          the result of the regex matching and using references to the regex groups(m1, m2, ...), IEs will be generated.

          The 'name' parameter needs to contain a string with interpolation placeholders and used to generate the name of
          the new IE. If the value is not present, no grouping of files into IEs will be performed. Likewise the 'label'
        ...
        def pre_process(item, *_args)
          stop_recursion if check_item_type(item, Teneo::Ingester::IntellectualEntity, raise_on_error: false)
          super
        end
