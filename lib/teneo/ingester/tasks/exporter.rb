    ...
    module Tasks
      class Exporter < Teneo::Ingester::Tasks::Base::Task
        ...
        help_text <<~STR
                    ...
                    If the 'export_mail' parameter is filled in, it is expected to contain a comma-separated list of email addresses 
                    ...
                    parameter. The value is generated by interpolating the given string using the 
                    [Kernel#sprintf](https://ruby-doc.org/core/Kernel.html#method-i-sprintf) syntax. The item's properties can be
                    referenced by their respective names.

                    Extra columns can be defined in the 'extra_keys' parameter. It needs to contain a Hash with header names as
        ...
        def process(item, *_args)
          case item
          when Teneo::DataModel::Collection
            export_collection(item)
          when Teneo::DataModel::IntellectualEntity
            export_item(item)
            stop_recursion
          else
            # do nothing
        ...
        def post_process(item)
          return unless item.is_a?(Teneo::Workflow::Job)
          attachments = []
        ...
        # @param [Teneo::DataModel::IntellectualEntity] item
        ...
        def get_key(export_file, item)
          ...
          item.interpolate(parameter(:export_key))
        end
