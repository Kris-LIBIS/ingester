    ...
    module Tasks
      ...
      class ChecksumTester < Teneo::Ingester::Tasks::Base::Task
        ...
        def check_exists(item)
          raise Teneo::Ingester::WorkflowError, "File '#{item.fullpath}' does not exist." unless File.exists? item.fullpath
        end
        ...
        def check_checksum(item)
          ...
          if checksum_type.nil?
            ...
            if checksumfile_path
              ...
              raise Teneo::Ingester::WorkflowError, "#{checksum_type} checksum file test failed for #{item.filepath}."
            else
        ...
        def test_checksum(item, checksum_type, expected = nil)
          ...
          set_item_status(status: :failed, item: item)
          raise Teneo::Ingester::WorkflowError, "Calculated #{checksum_type} checksum does not match expected checksum."
        end
