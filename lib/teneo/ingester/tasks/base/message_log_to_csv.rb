# frozen_string_literal: true

require 'csv'

module Teneo
  module Ingester
    module Tasks
      module Base

        module MessageLog2Csv

          CSV_HEADER = %w'Code Date Time Status Task Item Message'

          def log2csv(run, csv_file = nil, options = {})
            log_in = run.message_logs
            csv_out = csv_file ? File.open(csv_file, 'w') : StringIO.new
            log2csv_io(log_in, csv_out, options)
            return csv_out if StringIO === csv_out
            csv_out.close
          end

          # @param [IO] log_in
          # @param [IO] csv_out
          # @param [Hash] options
          def log2csv_io(log_in, csv_out = nil, options = {})
            csv_out ||= StringIO.new
            buffer = CSV_HEADER
            write_buffer_to_csv(buffer, csv_out, options)
            log_in.each do |entry|
              msg = entry.message
              msg = ([msg] + (entry.data[:stack_trace] || [])).join("\n") if entry.data[:trace]
              buffer = [
                  entry.severity.to_s.first,
                  entry.created_at.to_date,
                  entry.created_at.to_time,
                  entry.severity.to_s,
                  entry.task,
                  entry.data[:item_name],
                  msg
              ]
              write_buffer_to_csv(buffer, csv_out, options)
            end
          ensure
            csv_out.rewind
            csv_out
          end

          protected

          def write_buffer_to_csv(buffer, csv_out, options)
            return if buffer.empty? || buffer.all? { |x| x.strip.empty? }
            return if buffer != CSV_HEADER && options[:filter] && !options[:filter].upcase.include?(buffer[0])
            csv_out.puts(
                CSV.generate_line(
                    (options[:skip_date] ?
                         [buffer[3], buffer[4], buffer[5], buffer[6]] :
                         [buffer[3], buffer[1], buffer[2], buffer[4], buffer[5], buffer[6]]
                    ),
                    col_sep: ';', quote_char: '"'
                )
            )
          end

        end

      end
    end
  end
end
