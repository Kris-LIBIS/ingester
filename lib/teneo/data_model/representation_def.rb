...
module Teneo::DataModel
  ...
  class RepresentationDef < BaseSorted
    self.table_name = 'representation_defs'
    ...
    belongs_to :from, class_name: 'RepresentationDef', inverse_of: :dependencies, optional: true
    has_many :dependencies, class_name: 'RepresentationDef', foreign_key: :from_id, inverse_of: :from, dependent: :nullify

    has_many :conversion_workflows, -> { rank(:position) }, inverse_of: :representation_def, dependent: :destroy
    ...
    def self.from_hash(hash, id_tags = [:ingest_model_id, :label])
      ...
      item = super(hash, id_tags) do |item, h|
        if (from = h.delete(:from))
          item.from = record_finder Teneo::DataModel::RepresentationDef, from_id: hash[:ingest_model_id], label: from
        end
      ...
      if conversion_workflows
        ...
        conversion_workflows.each do |conversion_workflow|
          item.conversion_workflows <<
              Teneo::DataModel::ConversionWorkflow.from_hash(conversion_workflow.merge(representation_def_id: item.id))
        end
