...
module Teneo
  module Ingester
    ...
    class ImgageConverter < Teneo::Ingester::Converter
      include Teneo::Ingester::Base::Mailer
      ...
      parameter item_types: %w(Teneo::Ingester::IntellectualEntity Teneo::Ingester::Collection)
      ...
      def email_report(item, *attachments)
        ...
        warn "Ingest report could not be sent by email. The report can be found here: #{get_export_file(item)}", item
      rescue Exception => e
        error "Problem encountered while trying to send report by email: #{e.message} @ #{e.backtrace[0]}. " +
                  "The report can be found here: #{get_export_file(item)}", item
      end
