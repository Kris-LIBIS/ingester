    ...
    module Tasks
      class ManifestationBuilder < Teneo::Ingester::Tasks::Base::Task
        ...
        def process(item, *_args)
          status_progress(item: item, progress: 0, max: item.ingest_model.representation_defs.count)
          ...
          item.ingest_model.representation_defs.each do |representation_def|
            ...
            unless rep
              ...
              rep = Teneo::DataModel::Representation.new
              rep.representation_info = representation_def.representation_info
              rep.access_right = representation_def.access_right
              rep.name = representation_def.name
              #rep.label = representation_def.label
              rep.parent = item
              rep.save!
            end
        ...
        # @param [Teneo::DataModel::Representation] rep representation that is being created
        # @param [Teneo::DataModel::RepresentationDef] rep_def representation definition
        def build_representation(rep, rep_def)
          ...
          rep_def.conversion_workflows.each do |workflow|
            ...
            if group
              ...
              group = Teneo::DataModel::ItemGroup.new(name: workflow.name)
              rep << group
